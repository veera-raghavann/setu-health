...
function App(){
 ...
 const actionType=session?.nextAction?.type||session?.next_action?.type||"";
 const intakeComplete=actionType==="COMPLETE_SECTION"||session?.state==="history_captured";
 ...
 const send=async(value:string,mode="text")=>{if(!session||!value.trim())return;setLoading(true);setNotice("");try{
   ...
   if(action==="OPEN_RECORD_UPLOAD")setPage("records");
   else if(action==="OPEN_ABHA_CONNECT")setPage("abha");
 ...
 const inspectRecord=async(id:string)=>{setLoading(true);setNotice("");try{const existing=await fetch(API+"/v1/resources/"+id+"/evidence");if(existing.ok){const evidence=await existing.json();if(Array.isArray(evidence)&&evidence.length){setOcrResults(prev=>({...prev,[id]:{ocr:{engine:"Previously processed",languageHints:[],pages:[],status:"completed"},evidence}}));setExpandedRecord(id);return}}const r=await fetch(API+"/v1/resources/"+id+"/process?language_hint="+encodeURIComponent(language),{method:"POST"});const data=await r.json();if(!r.ok)throw new Error(data.error||"OCR processing failed");setOcrResults(prev=>({...prev,[id]:data}));setExpandedRecord(id)}catch(e:any){setNotice(e.message||"Unable to inspect OCR output")}finally{setLoading(false)}};
 ...
 return <div className="app">
  ...
  {page==="intake"&&<main className="intake"><div className="intakeTop"><div><p className="eyebrow">CURRENT HEALTH ISSUE</p><span className="languageChip">{langName(language)} · {voiceProvider}</span></div><button className="listenBtn" onClick={speakQuestion}>🔊 Hear question</button></div><div className="questionCard"><div className="aiDot">✦</div><h2>{question}</h2><p className="muted">Answer naturally. PRISM updates the case state and chooses the next question.</p></div>
   {intakeComplete?<div className="completionCard"><h3>Initial intake complete</h3><p>Your responses are now part of this session's patient-reported context. You can continue building the health profile or add supporting medical records.</p><div className="completionActions"><button className="primaryBtn" onClick={()=>setPage("profile")}>View My Health →</button><button onClick={()=>setPage("records")}>Add medical records →</button><button onClick={()=>{setSession(null);setPage("home")}}>Finish for now</button></div></div>:<>
